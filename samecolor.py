...
ORS_API_KEY = "eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6ImExYTkzZWI5MzgxMzRjYmNhY2VkNWY2ZmRhZWY1OWFhIiwiaCI6Im11cm11cjY0In0="   # optional: keep blank unless you want ORS routing
BUFFER_METERS = 10.0
PORT = 5000
# ----------------------------------------
...
function addPotholeMarkers(){
  ...
  potholes.forEach((p, idx) => {
    const m = L.circleMarker([p.lat, p.lon], {
      ...
      fill: true, 
      fillOpacity: 0.9
    }).bindPopup(`<b>pothole</b><br>lat:${p.lat}<br>lon:${p.lon}`);
...
function resetMarkers(){
  markers.forEach(m => {
    m.setStyle({color:'blue', fillColor:'blue'});
  });
}
...
def add_pothole():
    ...
    collection.insert_one({"lat": lat, "lon": lon, "tag": tag})
