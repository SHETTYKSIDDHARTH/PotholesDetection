...
# Modified so:
# - On startup, any existing documents without a "source" field are marked {"source":"old"}
# - New inserts (via /add_pothole) get {"source":"new"}
# - Map colors: old = blue, new = green
# - Highlights of potholes near route remain red (as before)
#
# Run: python3 interactive_route_server.py
# Make sure pymongo, flask, shapely, pyproj, requests are installed.
...
ORS_API_KEY = "eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6IjVlY2EwODYwMjI0YjRlYjE4YmVmYzdiODc3MmEyNWYzIiwiaCI6Im11cm11cjY0In0="   # optional: put your ORS key if you want routing; leave blank to use straight route
BUFFER_METERS = 10.0
PORT = 5000
# ----------------------------------------
...
# Mark existing documents as "old" if they lack the `source` field
# This runs once on startup and is idempotent.
collection.update_many({"source": {"$exists": False}}, {"$set": {"source": "old"}})
...
def get_all_potholes():
    ...
    for p in docs:
        potholes.append({
            "lat": float(p["lat"]),
            "lon": float(p["lon"]),
            "tag": p.get("tag", "pothole"),
            "severity": p.get("severity", None),
            "source": p.get("source", "old")
        })
    return potholes
...
// returns color based on source: old -> blue, new -> green
function colorForSource(src) {
  if(!src) return 'blue';
  const s = String(src).trim().toLowerCase();
  if (s === 'new') return 'green';
  // anything else including 'old' => blue
  return 'blue';
}
...
function addPotholeMarkers(){
  ...
  potholes.forEach((p, idx) => {
    const baseColor = colorForSource(p.source);
    const m = L.circleMarker([p.lat, p.lon], {
      ...
      fill: true, 
      fillOpacity: 0.9
    }).bindPopup(`<b>${p.tag||p.severity||'pothole'}</b><br>lat:${p.lat}<br>lon:${p.lon}<br>source:${p.source||'old'}`);
...
function resetMarkers(){
  markers.forEach(m => {
    const idx = m._pothole_index;
    const p = potholes[idx];
    const baseColor = colorForSource(p.source);
    m.setStyle({color: baseColor, fillColor: baseColor});
  });
}
...
def add_pothole():
    ...
    try:
        ...
        return jsonify({"error":"lat required (number)"}), 400
    try:
        lon = float(data["lon"])
    except Exception:
        return jsonify({"error":"lon required (number)"}), 400
    # always mark new inserts with source="new"
    tag = data.get("tag", "pothole")
    collection.insert_one({"lat": lat, "lon": lon, "tag": tag, "source": "new"})
