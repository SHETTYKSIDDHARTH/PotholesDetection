...
function addPotholeMarkers(){
  ...
  potholes.forEach((p, idx) => {
    const m = L.circleMarker([p.lat, p.lon], {radius:6, color:'blue', fill:true, fillOpacity:0.9})
      .bindPopup(`<b>${p.tag||'pothole'}</b><br>lat:${p.lat}<br>lon:${p.lon}`);
...
async function doRouteAndCount(){
  ...
  document.getElementById('countSpan').innerText = j.inside_indices.length;
